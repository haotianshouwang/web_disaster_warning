...
> **🔌 独立 Web 仪表盘**：本项目现已支持独立的 Web 仪表盘前端，通过 WebSocket 连接后端获取全部实时数据（运行状态/事件列表/数据统计/实时推送），纯只读安全设计。仪表盘前端为独立项目，可部署到任意静态服务器。！！！本仓库为独立 Web 版，与主仓库区分，请注意拉取路径！！！
...
支持以下通知通道，各通道独立配置、热重启、支持图片：

| 通道 | 说明 | 图片支持 |
|---|---|---|
| 🖥️ CLI 终端 | 独立模式实时输出，不过滤全量展示 | ✅ 文本显示 |
| 📊 Web 仪表盘 | WebSocket 实时推送，含缩略图预览 | ✅ base64 + URL |
| 📱 OneBot 11 (QQ) | 四种协议模式（HTTP 服务器/客户端、WS 服务器/客户端） | ✅ CQ 码 `[CQ:image,file=base64://...]` |
| 📧 邮件 (SMTP) | 多收件人 + 发件人名称，内嵌图片 | ✅ MIME multipart + URL |

- **独立于 AstrBot 会话体系**：QQ 和邮件推送不依赖 AstrBot 框架，插件直连 NapCat/LLOneBot
- **事件类型过滤**：各地震 🌍 / 气象 ⛈️ / 海啸 🌊 可单独开关
- **多目标推送**：QQ 支持多群 + 多私聊组合，邮件支持多收件人
- **热重启**：配置变更后自动重启适配器，无需重启整个插件
- **卡片渲染**：USGS/Global Quake 地震事件自动渲染信息卡片图片
- **气象图标**：中国气象局预警自动附带 NMC 图标
...
| 模式 | 方向 | 默认端口 |
|---|---|---|
| HTTP 服务器 | NapCat → 插件 | 5700 |
| HTTP 客户端 | 插件 → NapCat | — |
| WS 服务器 | NapCat → 插件（反连）| 5701 |
| WS 客户端 | 插件 → NapCat | — |

- 每种模式独立配置 Access Token
- 发送通道优先级：WS Client → WS Server → HTTP Client
- WS Server 模式下双向复用同一连接收发消息
