"""
统计加载服务。
负责 StatisticsManager 的 JSON / 数据库加载、历史迁移与内存状态恢复，
...
class StatsLoadService:
    ...
    async def load(self) -> None:
        """加载统计数据。"""
        # 当前加载顺序是“先 JSON 补基础状态，再数据库恢复 recent_pushes 与聚合结果”，
    ...
    async def migrate_json_from_file(self) -> None:
        ...
        try:
            ...
            self.manager.repository.write_stats_file(saved_stats)
        except Exception as e:
    ...
    def _cleanup_json_recent_pushes(self) -> None:
        try:
            ...
            if saved_on_disk.get("recent_pushes"):
                ...
                self.manager.repository.write_stats_file(saved_on_disk)
